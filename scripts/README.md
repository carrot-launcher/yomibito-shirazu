...
| コマンド | 説明 |
|---|---|
| `node monitor.js latest [TYPE] [N]` | 最新 N 件（デフォルト50）を全歌会から表示。TYPE は `posts`（既定）/ `comments` / `all` |
| `node monitor.js watch [TYPE]` | 新着をリアルタイム表示（Ctrl+C で終了）。TYPE は同上 |
| `node monitor.js hogo [TYPE]` | 裁き済み（反故）コンテンツの一覧。TYPE は同上 |
...
```
[2026-04-22 10:33:12] 歌 [β歌会 / AQOAC780doEPKI5yIPSR] [山田#123456 / 1K89gOkDY3g...] 春霞かかる山辺に立つ朝よ…
[2026-04-22 10:34:00] 評 [β歌会 / AQOAC780doEPKI5yIPSR] [佐藤#987654 / abc...def] →[春霞かかる山辺に立つ…] いい歌ですね
[2026-04-22 22:17:41] 歌 [ヤバいヤツ / xyz123] [匿名#234567 / uid999] [裁き:caution(無礼) / 解題:山田#123456] (反故)
```

- 時刻（ローカル）
- 種別: `歌` または `評`
- `[歌会名 / groupId]`
- `[displayName#userCode / uid]` — `suspend <uid>` などにそのまま使える
- フラグ: `裁き:type(理由)`, `解題:名前#ID` など（該当時のみ）
- 評の場合: `→[対象の歌本文先頭20字]` が本文の前に付く
- 本体（裁き済みは `(反故)`）
